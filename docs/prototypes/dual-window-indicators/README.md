...
Choose one direction, or name a combination of parts to keep. The decision should account for: provider identity, both usage windows, the most urgent pace status, stale/error states, and window-keeping activity. The next version will replace the illustrative `C`/`A` values with the provider-neutral domain model once that decision exists.
